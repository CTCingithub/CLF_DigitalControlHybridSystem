...
$$
\vec{u} = K_{0}(\mathrm{Gain}) \vec{x}_{N} + K_{1}(\mathrm{Gain}) \vec{x}_{N - 1} + \cdots + K_{M}(\mathrm{Gain}) \vec{x}_{N -M},\qquad T \in (N, N + 1), N \in \mathbb{Z}^{+}
$$
...
其中，系统矩阵$\mathcal{A}$和输入矩阵$\mathcal{B}$与系统自身有关，$\mathcal{B}_{i}(i \in \mathbb{Z}^{+} \cap [1,M],M \in \mathbb{Z}^{+})$与输入矩阵$\mathcal{B}$以及增益的组合$\mathrm{Gain}$有关。注意这里的$\mathrm{Gain}$是表示增益组合的元组，不是某一个矩阵。
...
$$
\frac{d \vec{x}_{T}^{\top}}{d T} \frac{H}{2} \vec{x}_{T} + \vec{x}_{T}^{\top} \frac{H}{2} \frac{d \vec{x}_{T}}{d T} + \frac{\lambda}{2} \vec{x}_{T}^{\top} H \vec{x}_{T}
$$
